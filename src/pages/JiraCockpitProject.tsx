  ...
  const loadProjectData = async () => {
    try {
      ...
      if (configData) {
        ...
        // Load AI insights for this specific project
      ...
      <Tabs defaultValue="insights" className="space-y-6">
        ...
        <TabsContent value="insights" className="space-y-6">
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            {/* Critical Alerts - Full Width on Mobile, 2 cols on Desktop */}
                ...
                <CardContent>
                  {slaRiskIssues > 0 ? (
                    <div className="space-y-4">
                      {insights
                        .filter(insight => insight.insight_type === 'sla_risk' && insight.confidence_score > 0.7)
                        .slice(0, 3)
                        .map((insight) => {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card className="glass-card">
              ...
              <CardContent>
                {insights.filter(i => i.insight_type === 'sprint_prediction').length > 0 ? (
                  <div className="space-y-4">
                    {insights
                      .filter(i => i.insight_type === 'sprint_prediction')
                      .slice(0, 2)
                      .map((prediction) => (
                  ...
                  <div className="text-center py-8 text-muted-foreground">
                    ...
                    <p className="text-sm">Execute ações rápidas para gerar insights</p>
                  </div>
                )}
              </CardContent>
            </Card>
                ...
                <div className="grid grid-cols-3 gap-4 text-center mb-4">
                  <div className="p-3 bg-destructive/10 rounded-lg">
                    <div className="text-2xl font-bold text-destructive">
                      {insights.filter(i => i.insight_type === 'sla_risk' && i.confidence_score > 0.7).length}
                    </div>
                    <div className="text-xs text-muted-foreground">Alto Risco</div>
                  </div>
                  <div className="p-3 bg-warning/10 rounded-lg">
                    <div className="text-2xl font-bold text-warning">
                      {insights.filter(i => i.insight_type === 'sla_risk' && i.confidence_score >= 0.4 && i.confidence_score <= 0.7).length}
                    </div>
                    <div className="text-xs text-muted-foreground">Médio Risco</div>
                  </div>
                  <div className="p-3 bg-success/10 rounded-lg">
                    <div className="text-2xl font-bold text-success">
                      {insights.filter(i => i.insight_type === 'sla_risk' && i.confidence_score < 0.4).length}
                    </div>
                    <div className="text-xs text-muted-foreground">Baixo Risco</div>
                  </div>
                </div>
                
                {insights.filter(i => i.insight_type === 'sla_risk').length === 0 && (
